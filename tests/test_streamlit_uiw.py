...
def test_add_character_validation(mock_st):
    ...
    _tab_characters()
    mock_st.error.assert_any_call("角色 ID 已存在")
...
@patch("sandbox_dating_sim.ui.streamlit_uiw.st")
def test_tab_flags_ending_delete_buttons(mock_st):
    """已有 ending 時，應為每個 ending render del_ending_<ending_id> 刪除按鈕。"""
    mock_st.columns.side_effect = lambda x, **kwargs: [MagicMock() for _ in range(len(x))] if isinstance(x, list) else [MagicMock() for _ in range(x)]
    mock_st.button.return_value = False
    mock_st.text_input.return_value = ""
    ...
    del_keys = [
        ...
        if call.kwargs.get("key", "").startswith("del_ending_")
    ]
    assert "del_ending_end_good" in del_keys
    assert "del_ending_end_bad" in del_keys
...
def test_add_ending_validation_duplicate_id(mock_st):
    ...
    assert any("alice" in c and "character" in c for c in error_calls)
    assert state["endings"] == []
