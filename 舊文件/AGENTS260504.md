...
每次新對話開始時，依下列分層順序閱讀，忽略 `舊文件/`：

**第一層（必讀，快速建立全貌）：**

1. `AGENTS.md`（本文件）
2. `PROJECT_BRIEF.md`（專案簡報，含架構、進度、規格索引）
3. `已知問題.md`（當前待辦與已知缺口）
4. `git log --oneline -15`（最近 commit）

**第二層（按當前任務展開，只讀相關區段）：**

- 依 `PROJECT_BRIEF.md` 的行號索引，讀取 `開發設計方針.md` / `測試指南.md` 中與當前任務相關的區段。
- 不要整份讀完，只讀需要的行範圍。

**第三層（實作時按需查閱）：**

- `Sandbox_Dating_Sim_初始設定_v1.2.md`：確認欄位定義、預設選項、UI 規格。
- `Sandbox_Dating_Sim_Dev_正式規格_v1.2.md`：確認系統架構、DSL 語法。
- `Sandbox_Dating_Sim_文件管線規格_v1.1.md`：確認文件格式。
- 原始碼：按任務需要讀取。
